 ...
 * 以前は最初の 1 つだけを見ていたが、`body` の余白を詰めているものより
 * 前に別の `@media` が現れた途端、「body の余白を詰めているか」を見ていた
 * テストがそちらを見て落ちた。#94（見本ページを狭い画面に収めるもの）と
 * #95（`.detail-list` を 1 列に落とすもの）が独立に同じ罠を踏んでいる。
